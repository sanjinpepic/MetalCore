...
const PerformanceMatrix = ({ steels, setDetailSteel, activeProducer, setActiveProducer, producers }) => {
    ...
    return (
        <div className="flex flex-col lg:flex-row flex-1 min-w-0 min-h-dvh lg:h-full bg-obsidian lg:overflow-hidden max-w-[100vw] overflow-x-hidden">
            ...
            <aside className="hidden lg:flex flex-col w-[400px] border-r border-white/5 bg-stone-950 overflow-y-auto no-scrollbar">
                <div className="p-8 space-y-8">
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500">
                        ...
                        <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
                            <span className="text-[10px] font-mono font-medium text-stone-500 uppercase tracking-widest">Labels</span>
                            <div className="flex gap-1">
                                {['none', 'super', 'all'].map(d => (
                                    <button
                                        key={d}
                                        onClick={() => setLabelDensity(d)}
                                        className={labelDensity === d ? 'px-2 py-1 rounded-md text-[9px] font-mono font-medium uppercase transition-colors bg-accent text-[#1A0C05]' : 'px-2 py-1 rounded-md text-[9px] font-mono font-medium uppercase transition-colors text-stone-500 hover:text-white'}
                                    >
                                        {d}
                                    </button>
                                ))}
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <span className="text-[10px] font-mono font-medium text-stone-500 uppercase tracking-widest">Y-Axis</span>
                            <div className="grid grid-cols-2 gap-2">
                                {Object.keys(axisOptions).map(key => (
                                    <button
                                        ...
                                        disabled={key === xAxis}
                                        className={yAxis === key
                                            ? 'px-3 py-2.5 rounded-xl text-[10px] font-mono font-medium uppercase tracking-wider transition-colors text-left bg-accent text-[#1A0C05] shadow-ember-sm'
                                            : 'px-3 py-2.5 rounded-xl text-[10px] font-mono font-medium uppercase tracking-wider transition-colors text-left bg-white/5 text-stone-400 border border-white/5 hover:bg-white/10'}
                                    >
                        ...
                        <div className="space-y-3">
                            <span className="text-[10px] font-mono font-medium text-stone-500 uppercase tracking-widest">X-Axis</span>
                            <div className="grid grid-cols-2 gap-2">
                                {Object.keys(axisOptions).map(key => (
                                    <button
                                        key={`x-${key}`}
                    ...
                    {displaySteel && (
                        <div className="space-y-6 pt-6 border-t border-white/5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-[10px] font-mono font-medium text-stone-500 uppercase mb-1 flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="p-3 bg-accent text-[#1A0C05] rounded-xl hover:scale-105 active:scale-95 transition shadow-ember-sm"
                                >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {Object.keys(axisOptions).map(key => (
                                    <div key={key} className={xAxis === key || yAxis === key ? 'p-3 rounded-2xl bg-black/40 border transition-colors border-accent/40 bg-accent/5' : 'p-3 rounded-2xl bg-black/40 border transition-colors border-white/5'}>
                                        <div className="text-[9px] font-mono font-medium text-stone-600 uppercase mb-1">{axisOptions[key].shortLabel}</div>
                            ...
                            <div className="flex gap-1 flex-1">
                                {Object.keys(axisOptions).map(key => (
                                    <button
                                        ...
                                        disabled={key === xAxis}
                                        className={yAxis === key
                                            ? 'flex-1 py-1 rounded-md text-[8px] font-mono font-medium uppercase tracking-tight transition-colors bg-accent text-[#1A0C05]'
                                            : key === xAxis
                                                ? 'flex-1 py-1 rounded-md text-[8px] font-mono font-medium uppercase tracking-tight transition-opacity bg-white/5 text-stone-600 opacity-30'
                                                : 'flex-1 py-1 rounded-md text-[8px] font-mono font-medium uppercase tracking-tight transition bg-white/5 text-stone-400 active:scale-95'}
                                    >
                ...
                <div ref={isFullScreen ? exportRef : containerRef} className={isFullScreen ? 'relative bg-obsidian !p-0' : 'relative px-2 lg:px-12 py-2 lg:py-10 lg:flex-1 lg:flex lg:flex-col lg:min-h-0'}>
                    <div ref={isFullScreen ? containerRef : exportRef} className={isFullScreen ? 'h-[50vh] lg:h-auto lg:flex-1 lg:min-h-0 glass-panel p-1.5 lg:p-12 relative overflow-hidden group/chart border-none rounded-none' : 'h-[50vh] lg:h-auto lg:flex-1 lg:min-h-0 glass-panel rounded-2xl lg:rounded-3xl p-1.5 lg:p-12 relative overflow-hidden group/chart border-white/10 hover:border-white/20 transition-colors'}>
                        ...
                        <button
                            ...
                            className="absolute top-4 right-4 z-50 p-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-stone-400 hover:text-white transition-opacity backdrop-blur-md opacity-0 group-hover/chart:opacity-100 hidden lg:block"
                                                        ...
                                                        <div className="grid grid-cols-2 gap-3">
                                                            {Object.keys(axisOptions).map(key => (
                                                                <div key={key} className={xAxis === key || yAxis === key ? 'p-2.5 rounded-xl bg-white/5 border border-accent/30' : 'p-2.5 rounded-xl bg-white/5 border border-white/5'}>
                                                                    <div className={xAxis === key || yAxis === key ? 'text-[8px] uppercase font-mono font-medium mb-1 text-accent-400' : 'text-[8px] uppercase font-mono font-medium mb-1 text-stone-500'}>{axisOptions[key].shortLabel}</div>
                    ...
                    {isFullScreen && (
                        <div className="absolute bottom-0 left-0 w-full bg-stone-950/80 backdrop-blur-3xl border-t border-white/10 p-3 flex flex-col gap-2 animate-in slide-in-from-bottom-full duration-700 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.5)] z-50">
                            <div className="flex items-center gap-6 px-4">
                                {/* Simple Search Module */}
                                <div className="relative w-80">
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500">
                                        <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
                                    </svg>
                                    <input
                                        type="text"
                                        placeholder="Search steels..."
                                        className="w-full bg-white/5 border border-white/10 rounded-lg py-1.5 pl-9 pr-3 text-white text-[10px] font-bold focus:outline-none focus:border-accent/40 transition-colors placeholder:text-stone-600"
                                        value={searchTerm}
                                        onChange={e => setSearchTerm(e.target.value)}
                                    />
                                </div>
                                ...
                                <div className="flex items-center gap-3 border-l border-white/5 pl-6 ml-2">
                                    <span className="text-[8px] font-mono font-medium text-stone-500 uppercase tracking-widest">Labels</span>
                                        ...
                                        {['none', 'super', 'all'].map(d => (
                                            <button
                                                ...
                                                className={labelDensity === d
                                                    ? 'px-2 py-1 rounded-md text-[8px] font-mono font-medium uppercase transition-colors bg-accent text-[#1A0C05] shadow-sm'
                                                    : 'px-2 py-1 rounded-md text-[8px] font-mono font-medium uppercase transition-colors text-stone-500 hover:text-white'}
                                            >
                                                {d}
                                            </button>
                                        ))}
                                        ...
                                        <div className="flex gap-1">
                                            {Object.keys(axisOptions).map(key => (
                                                <button
                                                    ...
                                                    disabled={key === xAxis}
                                                    className={yAxis === key
                                                        ? 'px-3 py-1.5 rounded-lg text-[9px] font-mono font-medium uppercase transition-colors bg-accent text-[#1A0C05] shadow-ember-sm'
                                                        : 'px-3 py-1.5 rounded-lg text-[9px] font-mono font-medium uppercase transition-colors bg-white/5 text-stone-500 hover:text-white'}
                                                >
                                                    {axisOptions[key].shortLabel}
                                                </button>
                                            ))}
                                ...
                                <div className="border-l border-white/5 pl-6">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-4 py-2 bg-accent text-[#1A0C05] rounded-xl font-mono font-medium text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition shadow-ember-sm"
                                    >
                                ...
                                {producers.map(prod => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={isActive
                                                ? 'flex items-center gap-1.5 px-2.5 py-1 rounded-md transition active:scale-95 bg-accent/10 border border-accent/25'
                                                : 'flex items-center gap-1.5 px-2.5 py-1 rounded-md transition active:scale-95 hover:bg-white/5 border border-transparent'}
                                        >
                            ...
                            <div className="bg-[#12100D]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-3 flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex gap-3 mt-0.5">
                                        <span className="text-[9px] text-stone-400 font-bold"><span className="text-accent-400">{axisOptions[yAxis].shortLabel}</span> {selectedSteel[yAxis]}</span>
                                        <span className="text-[9px] text-stone-400 font-bold"><span className="text-accent-400">{axisOptions[xAxis].shortLabel}</span> {selectedSteel[xAxis]}</span>
                                    </div>
                                </div>
                                <button
                                ...
                                <button
                                    ...
                                    className="p-2 bg-white/10 text-stone-400 rounded-xl shrink-0 active:scale-95 !min-w-0 !min-h-0"
                                >
