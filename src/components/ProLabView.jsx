...
const ProLabView = ({ steels }) => {
    ...
    const [snapshots, setSnapshots] = useState([]); // { name: string, data: array, hrc: number, temp: number }
    ...
    const collectionFingerprint = useMemo(() => {
        ...
        return Object.entries(metrics).map(([el, val]) => ({
            subject: elementFunctions[el] || el,
        ...
        <div className="flex flex-col flex-1 min-w-0 min-h-dvh md:h-full md:overflow-y-auto custom-scrollbar bg-slate-950 p-6 md:p-12">
            <header className="mb-12">
                ...
                <p className="mt-6 text-slate-400 max-w-2xl text-sm md:text-base leading-relaxed">
                    Welcome to the Pro Research Laboratory. Use these advanced simulators to analyze your collection's chemical DNA, predict heat-treat outcomes, and <span className="text-white font-bold italic">benchmark industrial workflows</span>.
                </p>
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => {
                                    const newSnapshot = {
                                        id: Date.now(),
                                        name: `${simSteel.name} @ ${temperTemp}${getTemperatureUnit(unitSystem)}`,
                                        data: [...simData],
                                        hrc: currentHrc,
                                        temp: temperTemp,
                                        color: snapshots.length === 0 ? '#10b981' : snapshots.length === 1 ? '#3b82f6' : '#ec4899'
                                    };
                                    if (snapshots.length < 3) setSnapshots([...snapshots, newSnapshot]);
                                }}
                                disabled={snapshots.length >= 3}
                                className="px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-white uppercase tracking-widest hover:bg-white/10 disabled:opacity-30 transition-all"
                            >
                                Save Snapshot
                            </button>
                            <select
                    ...
                    <div className="flex-1 min-h-0 mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={simData}>
                                ...
                                />
                                <Area type="monotone" name="Current" dataKey="hrc" stroke="#f59e0b" strokeWidth={4} fillOpacity={1} fill="url(#colorHrc)" />
                                    ...
                                    <div className="flex gap-3 mt-3">
                                        {snapshots.map(snap => (
                                            <div key={snap.id} className="flex items-center gap-1.5">
                                                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: snap.color }} />
                                                <span className="text-[9px] font-bold text-slate-500 uppercase">{snap.hrc} HRC</span>
                                                <button onClick={() => setSnapshots(prev => prev.filter(s => s.id !== snap.id))} className="text-slate-600 hover:text-white transition-colors">
