...
const HomeView = ({ setView, steels, setDetailSteel }) => {
    ...
    return (
        <div className="flex flex-col flex-1 min-w-0 h-screen overflow-y-auto custom-scrollbar bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black">
            ...
            {/* Hero Section */}
            <div className="p-8 md:p-16 pt-20 md:pt-24 space-y-6 max-w-5xl">
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                    <button
                        ...
                        className="px-8 py-4 bg-white text-black rounded-xl font-bold hover:bg-slate-200 transition-all flex items-center gap-2 group"
                    >
                        Browse Library
            ...
            {/* Stats Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-8 md:px-16 mb-16 max-w-6xl">
                {stats.map((stat, i) => (
                ...
                <div className="glass-panel p-6 rounded-2xl border-white/5 bg-gradient-to-br from-accent/20 to-transparent border-accent/20">
                    <div className="text-3xl font-black text-accent mb-1 font-display">2.0</div>
                    <div className="text-[10px] font-bold text-accent/80 uppercase tracking-widest">Version</div>
                </div>
            </div>
            ...
            {featuredSteel && (
                <div className="px-8 md:px-16 pb-16">
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-4">
                        <span className="h-px w-8 bg-slate-800"></span>
                        Featured Alloy
                        <span className="h-px flex-1 bg-slate-800"></span>
                    </div>

                    <div className="glass-panel p-8 md:p-12 rounded-[2.5rem] border-white/5 flex flex-col md:flex-row gap-12 group hover:border-white/10 transition-all relative overflow-hidden">
                        <div className="md:w-1/2 relative z-10">
                            <div className="text-accent font-bold uppercase tracking-widest text-xs mb-2">{featuredSteel.producer}</div>
                            <h2 className="text-4xl md:text-5xl font-black text-white italic mb-6">{featuredSteel.name}</h2>
                            <p className="text-slate-400 mb-8 leading-relaxed">{featuredSteel.desc}</p>

                            <button
                                onClick={() => setDetailSteel(featuredSteel)}
                                className="inline-flex items-center gap-2 text-sm font-bold text-white border-b border-accent pb-1 hover:text-accent transition-colors"
                            >
                                View Data Sheet
                        ...
                        <div className="md:w-1/2 grid grid-cols-2 gap-4 relative z-10">
                            {['edge', 'toughness', 'corrosion', 'sharpen'].map(metric => (
                                <div key={metric} className="bg-black/40 rounded-2xl p-4 border border-white/5">
                                    <div className="text-[8px] text-slate-500 uppercase font-black mb-2">{metric}</div>
                                    ...
                                    <div className="text-right text-xs font-mono font-bold text-white mt-1">{featuredSteel[metric]}/10</div>
                                </div>
                            ))}
                        </div>
